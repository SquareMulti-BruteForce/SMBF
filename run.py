...
def banner():
	print (''' %s 
  Group%s \n __________       _____.__  \n \____    /____ _/ ____\__| %s> %sZero \n   /     /\\__  \\\   __\|  | %s> %sArt \n  /     /_ / __ \|  |  |  | %s>%s Fast \n /_______ (____  /__|  |__| %s>%s International \n         \/    \/ \n %s[%s*%s] By : %sDarckCode %s[%s*%s] facebook : Tiara Pandu \n %s[%s*%s] -------------------------------------- \n [%s*%s] IP : %s%s'''%
 (H,K,H,K,H,K,H,K,H,K,P,K,P,H,P,K,P,K,P,H,IP))
# MASUK TOKEN (TOKEN BUAT CRACK AKUN)
...
def masuk():
    ...
    if rom in(""):
        ...
        if romz in(""):
        	print("%s [!] Isi yang benar Goblok "%(M));exit()
    	try:
        ...
        if nanya in(""):
        	...
        	print ("\n%s [!] anda sungguh Goblok "%(M));jeda(2);os.system("xdg-open https://youtu.be/IG5QfdxRkeY");masuk()
    elif rom in ('0', '00'):
    	exit('\n')
    else:
    	print("%s [!] Isi yang benar Goblok "%(M));exit()
